...
| 1. Configs | `config/settings.yaml`, `config/risk.yaml`, `config/logging.yaml`, safe source-policy YAML files | Bootstrap | Default mode is simulation, bankroll is `$50`, forbidden modes fail validation | Reject unsafe config drift and use explicit policy defaults | Validated Python loader and TOML name variants |
...
| 5. Simulation broker | `execution/models.py`, `execution/portfolio_state.py`, `execution/simulation_broker.py` | Risk engine, storage | Only risk-approved simulated instructions change paper portfolio state | Reject unapproved or inconsistent broker requests | Live broker integrations |
...
- Separate editable YAML defaults from hard prohibitions.
...
- `config/settings.py`, TOML config-name variants, and the superseded `execution/portfolio.py` name wait until a post-MVP config and ledger naming pass is justified.
